...
export function mapKimiLine(parsed) {
  ...
  if (role === 'assistant') {
    ...
    // Mark as _resultSummary so the last text event becomes the tool return value
    // (Kimi has no dedicated end-of-run event like Claude Code's "result" type)
    if (typeof content === 'string' && content) {
      events.push({ type: 'text', text: content, _resultSummary: content });
    } else if (Array.isArray(content)) {
      for (const block of content) {
        if (typeof block === 'string' && block) {
          events.push({ type: 'text', text: block, _resultSummary: block });
        } else if (block?.type === 'text' && block.text) {
          events.push({ type: 'text', text: block.text, _resultSummary: block.text });
        }
      }
    }
